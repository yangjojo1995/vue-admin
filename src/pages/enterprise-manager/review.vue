<template>
  <div class="page">
    ...
    >
      ...
      >
        <span slot-scope="{ row }">
          <el-tooltip
            class="item"
            effect="dark"
            :content="row.creditCode"
            placement="top-end"
          >
            <span class="ellipsis">{{ row.creditCode || "-" }}</span>
          </el-tooltip>
        </span>
      </ym-table-column>
      ...
      <ym-table-column prop="email" label="联系邮箱" min-width="140px">
      ...
      <ym-table-column prop="createTime" label="申请时间" min-width="160px">
        <span slot-scope="{ row }">{{
...
export default {
  ...
  methods: {
    ...
    async requestService (paging, entity, orders) {
      return this.$service.manager.enterpriseAudit({
        ...paging,
        page: paging.page,
        ...entity,
        orders
    ...
    handleUpdateItem (row) {
      this.$router.push({
        ...
        query: { id: row.id, type: 'enterprise' }
      })
    },
