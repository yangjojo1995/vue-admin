<template>
  <el-card v-loading="loading">
    ...
    <div v-if="projectAudit.status&&(projectAudit.status==='refuse'||projectAudit.status==='c_refuse')" class="msg-content pro-reason">
      <p>
        很遗憾，您的项目未能通过审核，请根据推荐单位意见，修改后再次尝试提交。
      </p>
      <p>审核单位：{{projectAudit.orgName||'-'}}</p>
      <p>审核批注：{{projectAudit.reason||'-'}}</p>
    </div>
    <div class="msg-block">
      <div class="b-title">
        基本信息
        ...
        <el-row :gutter="24">
          <el-col :span="24">
            <ym-form-item label="项目亮点" prop="advantage">
              <ym-input v-trim type="textarea" v-model="projectBase.advantage" :maxlength="2000" placeholder="填写须知：从产品、技术、服务、市场、团队等方面，简要列示投资亮点"></ym-input>
            </ym-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="24">
          <el-col :span="24">
            <ym-form-item label="市场分析" prop="marketAnalysis">
              <ym-input v-trim type="textarea" v-model="projectBase.marketAnalysis" :maxlength="2000" placeholder="填写须知：截止到目前为止，营收、客户量、分布区域、合作资源、投资情况等"></ym-input>
            </ym-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="24">
          <el-col :span="24">
            <ym-form-item label="商业模式" prop="businessModel">
              <ym-input v-trim type="textarea" v-model="projectBase.businessModel" :maxlength="2000" placeholder="请简要介绍公司的商业模式"></ym-input>
            </ym-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="24">
        ...
        <el-row :gutter="24">
          ...
          <el-col :span="12">
            <ym-form-item class="long-label" label="统一社会信用代码" prop="creditCode">
              <ym-input v-model="projectCompany.creditCode" :maxlength="20" placeholder="请输入统一社会信用代码"></ym-input>
            </ym-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="24">
        ...
        <el-row :gutter="24">
          ...
          <el-col :span="12">
            <ym-form-item label="主营业务" prop="mainBusiness">
              <ym-input v-trim type="textarea" v-model="projectCompany.mainBusiness" :maxlength="2000" placeholder="请输入主营业务，不超过2000字"></ym-input>
            </ym-form-item>
          </el-col>
        </el-row>
      </ym-form>
        ...
        <el-row :gutter="24">
          <el-col :span="24">
            <ym-form-item label="产品&服务描述" prop="descriptions">
              <ym-input v-trim type="textarea" v-model="projectProduct.description" :maxlength="2000" placeholder="请输入"></ym-input>
            </ym-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="24">
            ...
            <ym-table-column prop="profile" label="简介" min-width="120px">
              <div slot-scope="{ row, $index }">
                <el-form-item :prop="'projectMembers.' + $index + '.profile'">
                  <ym-input v-trim type="textarea" v-model="row.profile" :maxlength="2000" placeholder="请输入"></ym-input>
                </el-form-item>
              </div>
            </ym-table-column>
    ...
    <!-- <div class="msg-block">
      ...
      >
        <el-row :gutter="24">
          <el-col :span="8">
            <ym-select
              v-model="projectBase.orgId"
              filterable
              remote
              style="width:100%"
              :remote-method="rgIdremoteMethod"
              placeholder="请选择"
            >
              <ym-option
  ...
  methods: {
    ...
    async loadData (id) {
      ...
      if (this.$route.query.entry === 'drafts') {
        result = await this.$service.project.draftInfo({
          id: id
        })
      } else {
        result = await this.$service.project.projectAuditDetail({
          id: id
        })
      }
      if (result) {
      ...
      if (this.$route.query.entry !== 'drafts') {
        // 编辑单独查询BP信息
        this.bpMsg = await this.$service.project.confirmedLatestBp({
    ...
    async changeParams () {
      ...
      if (this.$route.query.status) {
        ...
        const statusParams = { projectAudits: [{ id: this.id, status: this.$route.query.status, content: content }] }
        await this.$service.project.auditUpdate(statusParams)
      } else if (!this.id || this.$route.query.entry === 'drafts') {
        await this.$service.project.insertProjectDetails(params)
      } else {
        await this.$service.mine.updateProjectDetails(params)
      }
