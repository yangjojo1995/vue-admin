<template>
  <div class="page">
    ...
    <ym-table-list-card row-key="accountId" :loading="loading" :data="records" :current-page.sync="paging.page" :page-size.sync="paging.size" :total="total" @current-change="updateRecordsByPage" @size-change="updateRecordsByPage(1)" @sort-change="sortRecords">
      ...
      <ym-table-column prop="orgName" label="指定推荐单位" min-width="140px">
        <span slot-scope="{ row }">{{ row.orgName || "-" }}</span>
      </ym-table-column>

      <ym-table-column prop="publisherName" label="发布人" min-width="120px">
      ...
      <ym-table-column prop="createTime" label="创建时间" min-width="140px">
      ...
      <ym-table-column label="操作" min-width="120px" fixed="right">
        <template slot-scope="{ row }">
          <ym-link type="primary" v-if="row.status==='check'" @click="audit(row)">审核</ym-link>
          <ym-link type="primary" v-if="row.status==='refuse'" @click="views(row)">查看</ym-link>
        </template>
      </ym-table-column>
    </ym-table-list-card>
  </div>
  ...
  methods: {
    ...
    async requestService (paging, entity, orders) {
      if (entity && entity.status) {
        ...
        entity.statusList = ['check', 'refuse', 'revoke']
      }
    ...
    views (row) {
      this.$router.push({
        path: '/project/review/view',
        query: { id: row.id }
      })
    },
    audit (row) {
      this.$router.push({
        path: '/project/review/audit',
        query: { id: row.id }
      })
    }
  }
